...
describe('Cryptosabers contract', async function()  {
    ...
    describe('Failure Test',async () => {
        it('Mint should not work with third party address', async function()  {
            await expect(cryptosabers.connect(addr3).mint(10,
                0,"0x3031303230333034000000000000000000000000000000000000000000000000",
                "0x3031303230333034000000000000000000000000000000000000000000000000")).to.be.revertedWith('invalidaccess');
        });

        it('should not Mint Cryptosabers because quantity is greater than 2', async function()  {
            await expect( cryptosabers.mint(10,
                0,"0x3031303230333034000000000000000000000000000000000000000000000000",
                "0x3031303230333034000000000000000000000000000000000000000000000000"),
                { value: ethers.utils.parseEther("1") }).to.be.revertedWith('q<=2');
        });

        it('should not Mint Cryptosabers because quantity is 0', async function()  {
            await expect( cryptosabers.mint(0,
                0,"0x3031303230333034000000000000000000000000000000000000000000000000",
                "0x3031303230333034000000000000000000000000000000000000000000000000"),
                { value: ethers.utils.parseEther("1") }).to.be.revertedWith('q>0');
        });
    });
    ...
    describe('Transfers',async () => {
        ...
        it('Other address should mint cryptosabers NFT', async function()  {
            await cryptosabers.connect(addr5)
            .mint(2,
                0,"0x3031303230333034000000000000000000000000000000000000000000000000",
                "0x3031303230333034000000000000000000000000000000000000000000000000"
                );
            
            //(2,{ value: ethers.utils.parseEther("1") });
    ...
    describe('Check Token owner',async () => {
        it('Owner assign URL to the NFT',async () => {
            await cryptosabers.setUriSuffix("www.Cryptosabers.sol");
        });

        it('Owner assign URL to the NFT',async () => {
            await cryptosabers.setBaseURI("www.Cryptosabers.sol");
        });

        it('Check Token owner', async function()  {
            expect(await cryptosabers.tokenOfOwnerByIndex(addr1.address,0)).to.equal(1);
        });

        it('Check Token owner', async function()  {
            expect(await cryptosabers.tokenURI(0)).to.equal("www.Cryptosabers.sol0www.Cryptosabers.sol");
        });
